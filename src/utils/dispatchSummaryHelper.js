...
/**
 * Decrements the TblDispatchMonthly summary document when a dispatch is deleted.
 */
export async function decrementMonthlySummary(dispatch) {
    try {
        const date = dispatch.DispatchDate instanceof Date
            ? dispatch.DispatchDate
            : new Date(dispatch.DispatchDate);
        ...
        console.log(`📊 Monthly summary decremented → ${docId}`);
    } catch (err) {
        console.error("decrementMonthlySummary error:", err);
    }
}
